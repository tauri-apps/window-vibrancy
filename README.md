...
#[cfg(target_os = "macos")]
apply_vibrancy(&window, NSVisualEffectMaterial::HudWindow, Some(NSVisualEffectState::Active)).expect("Unsupported platform! 'apply_vibrancy' is only supported on macOS");
...
| `apply_vibrancy`                  | macOS 10.10 and newer | `state` is used to control vibrancy state, for default please use `None` and it will follow the window active state. |
